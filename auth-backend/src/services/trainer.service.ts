...
class TrainerService {
     ...
     */
    async getExploreCourses(filters: { search?: string } = {}) {
        ...
        return {
            courses: courses.map(c => {
                ...
                return {
                    ...
                    trainer: {
                        name: c.institute?.name ?? c.trainer?.name ?? staffTrainers[0]?.name ?? '—',
                        avatar: c.institute?.logo ?? c.trainer?.avatar ?? c.institute?.user?.avatar ?? null,
                    },
    ...
    async updateTrainerCourse(userId: string, courseId: string, data: any) {
        ...
        const updateData: any = {
            ...
            ...(data.status && {
                // capacity_based (حجز مرن) تُنشر مباشرة دون مراجعة الأدمن
                // فقط الدورات الحضورية (in_person) غير المرنة تحتاج مراجعة
                status: (data.status.toUpperCase() === 'ACTIVE' && data.deliveryType === 'in_person' && data.deliveryType !== 'capacity_based')
                    ? 'PENDING_REVIEW'
    ...
    async createCourse(userId: string, data: any, paymentReceiptPath?: string) {
        ...
        const course = await prisma.course.create({
            data: {
                ...
                // capacity_based (حجز مرن) تُنشر مباشرة Active دون مراجعة
                status: (data.status === 'ACTIVE' && sessionType === 'IN_PERSON' && data.deliveryType !== 'capacity_based')
                    ? 'PENDING_REVIEW'
                    : (data.status || 'DRAFT'),
                image: data.image,
