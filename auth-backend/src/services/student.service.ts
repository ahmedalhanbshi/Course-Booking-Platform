...
class StudentService {
    ...
    async getDashboard(userId: string) {
        ...
        const currentCourses = enrollments.map((e: any) => ({
            ...
            trainer: e.course.institute?.name || e.course.trainer?.name || ((e.course.staffTrainerIds as string[])?.length > 0 ? 'مدرب معهد' : 'مدرب'),
        ...
        return enrollments.map((e: any) => {
            ...
            const trainersList = staffIds.length > 0 
                ? staffIds.map(id => ({
                    ...
                    avatar: e.course.institute?.logo || null
                }))
                : (e.course.institute?.id 
                    ? [{
                        id: e.course.institute.id,
                        name: e.course.institute.name,
                        avatar: e.course.institute.logo || null
                    }]
                    : [{
    ...
    async getCourseDetails(userId: string, courseId: string) {
        ...
        return {
            ...
            instructor: (course as any).staffTrainerIds?.length > 0 ? {
                ...
                role: course.institute?.name ? `مدرب لدى ${course.institute.name}` : 'مدرب معهد',
                avatar: course.institute?.logo || null,
                ...
                role: 'جهة منظمة',
                avatar: course.institute.logo,
                email: course.institute.email,
                phone: course.institute.phone,
                bio: course.institute.description,
                specialties: []
            } : {
    ...
    async getWishlist(userId: string) {
        ...
        return wishlists.map(w => {
            ...
            return {
                ...
                category: course.category?.name || 'عام',
                trainer: {
                    name: course.institute?.name || course.trainer?.name || ((course as any).staffTrainerIds?.length > 0 ? 'مدرب معهد' : 'مدرب'),
                },
    ...
    async getSchedule(userId: string) {
        ...
        return sessions.map((s: any) => ({
            ...
            trainerName: s.course?.institute?.name || s.course?.trainer?.name || ((s.course as any)?.staffTrainerIds?.length > 0 ? 'مدرب معهد' : 'مدرب'),
            startTime: s.startTime,
            endTime: s.endTime,
