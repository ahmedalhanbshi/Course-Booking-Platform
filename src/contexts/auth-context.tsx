...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    const login = async (email: string, password: string): Promise<boolean> => {
        try {
            ...
            const errorMessage = getRequestErrorMessage(error, 'فشل تسجيل الدخول. يرجى التحقق من البيانات.')
    ...
    const register = async (data: RegisterData | FormData): Promise<boolean> => {
        try {
            ...
            const errorMessage = getRequestErrorMessage(error, 'فشل إنشاء الحساب. يرجى المحاولة مرة أخرى.')
            throw new Error(errorMessage)
        } finally {
            setIsLoading(false)
        }
