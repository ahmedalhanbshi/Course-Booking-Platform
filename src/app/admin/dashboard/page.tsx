...
import { Users, Building, BookOpen, DollarSign, TrendingUp, AlertTriangle, CheckCircle, Clock, ShieldCheck, ArrowUpRight, Activity } from "lucide-react"
...
const mockPendingItems = [
  ...
  },
  {
    ...
    description: "دورة تنتظر الموافقة",
    date: "2024-01-13"
  }
]
...
const mockRecentActivity = [
  {
    action: "اعتماد معهد",
    details: "تم اعتماد معهد الرياض للتدريب",
    time: "منذ ساعتين"
  },
  {
    action: "دفع فاشل",
    details: "دفع بقيمة 500 ريال يمني لم يكتمل",
    time: "منذ 3 ساعات"
  },
  {
    action: "تسجيل جديد",
    details: "انضم 5 طلاب جدد اليوم",
    time: "منذ 4 ساعات"
  }
]
...
export default function AdminDashboard() {
  ...
  const handleConfirmReject = () => {
    console.log("Rejected:", selectedRequest?.title, "Reason:", rejectionReason)
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between p-6 md:p-8 gap-6">
          <div className="space-y-3 text-center md:text-right max-w-2xl">
            <div>
              ...
              <p className="text-gray-300 text-sm md:text-base opacity-90">
                نظرة شاملة على أداء المنصة. لديك <span className="font-bold text-white">{mockStats.pendingApprovals} طلبات معلقة</span> تتطلب انتباهك.
              </p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex gap-4">
            <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 min-w-[100px]">
              <div className="text-2xl font-bold mb-0.5">{mockStats.totalUsers.toLocaleString('en-US')}</div>
              <div className="text-[10px] font-medium text-gray-400 uppercase">مستخدم</div>
            </div>
            <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 min-w-[100px]">
              <div className="text-2xl font-bold mb-0.5">{mockStats.totalInstitutes}</div>
            ...
            <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 min-w-[140px]">
              <div className="text-2xl font-bold mb-0.5">{mockStats.totalRevenue.toLocaleString('en-US')}</div>
          ...
          <Card className="border-none shadow-md bg-white overflow-hidden ring-1 ring-gray-100 rounded-xl">
            <CardHeader className="bg-orange-50/50 border-b border-orange-100 py-3 px-4 flex flex-row items-center justify-between">
              ...
              <Badge variant="secondary" className="bg-orange-100 text-orange-700 hover:bg-orange-200">
                {mockStats.pendingApprovals} جديد
              </Badge>
            </CardHeader>
