...
const statusOptions = ["كل الحالات", "قادمة", "جارية", "منتهية"]
const typeOptions = ["كل الأنواع", "حضوري", "أونلاين"]
...
const courses = [
  ...
  },
  {
    ...
    description: "منهج عملي لإدارة المشاريع باحترافية مع أدوات حديثة.",
    category: "إدارة أعمال",
    image: "/images/course-management.png",
    studentsCount: 0,
    duration: "غير محدد",
    trainer: {
      id: "trainer-3",
...
export default function InstituteExploreCoursesPage() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses
      .filter((course) => course.instituteId === mockInstitute.id)
      .filter((course) => {
        const matchesSearch =
          query.length === 0 ||
        ...
        const matchesStatus =
          selectedStatus === "كل الحالات" || course.status === selectedStatus

        const matchesType =
          selectedType === "كل الأنواع" || course.type === selectedType

        return matchesSearch && matchesCategory && matchesPrice && matchesStatus && matchesType
      })
  }, [searchQuery, selectedCategory, selectedPrice, selectedStatus, selectedType])
