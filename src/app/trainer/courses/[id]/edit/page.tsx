...
import { Save, Send, Trash2, ArrowLeft, X, MapPin, Users, Building, Globe, Plus, CalendarDays, CheckCircle, AlertCircle, Banknote, Lock, Loader2, UploadCloud } from "lucide-react"
...
export default function EditTrainerCoursePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const [data, hls, tags, cats] = await Promise.all([
                ...
                // Populate online sessions
                if ((data.deliveryType || '').toLowerCase() === 'online' && data.sessions?.length > 0) {
                    ...
                    if (data.sessions[0]?.location) {
                        ...
                        const matchedPlatform = platforms.find(p => p.value === loc)
                        setOnlineSchedule({
                            platform: matchedPlatform ? loc : (loc !== 'Online' ? 'other' : ''),
    ...
    const handleResubmit = async () => {
        ...
        try {
            ...
            setCourseData(prev => ({ ...prev, roomBooking: data.roomBooking || null }))
        ...
        try {
            ...
            if (isPublishingOrActive) {
                ...
                if (courseData.deliveryType === 'in_person') {
                    ...
                    if (!paymentFile && !courseData.roomBooking) throw new Error("يجب إرفاق سند الدفع لحجز القاعة")
    ...
    // التبويب متاح دائماً — نحدد ما يمكن تعديله داخله
    const isInPersonReadOnly = isActive && courseData.deliveryType === 'in_person'
    const canEditSchedule = canSetupSchedule || (isActive && courseData.deliveryType === 'online')
    // قسم الأونلاين يظهر دائماً عند اختيار أونلاين
    const showOnlineSection = courseData.deliveryType === 'online' || courseData.deliveryType === 'ONLINE'
    // قسم الحضوري يظهر دائماً عند اختيار حضوري
    const showInPersonSection = courseData.deliveryType === 'in_person' || courseData.deliveryType === 'IN_PERSON'
    ...
    return (
        <div className="max-w-5xl mx-auto pb-12" dir="rtl">
                ...
                <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                    <DialogTrigger asChild>
                        <Button variant="destructive" size="sm" disabled={isSubmitting}>
                            <Trash2 className="mr-2 h-4 w-4" />حذف الدورة
                        </Button>
                    </DialogTrigger>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 items-start">
                            <div className="relative h-40 w-full md:w-64 rounded-lg border-2 border-dashed border-red-200 overflow-hidden bg-white flex items-center justify-center cursor-pointer hover:bg-red-50" onClick={() => document.getElementById('resubmit-input')?.click()}>
                                {resubmitPreview
                                    ...
                                    : <div className="flex flex-col items-center text-gray-400"><UploadCloud className="h-8 w-8 mb-2" /><span className="text-xs">إرفاق سند جديد</span></div>
                                }
                                <input id="resubmit-input" type="file" className="hidden" accept="image/*" onChange={e => {
                                    if (e.target.files?.[0]) { setResubmitFile(e.target.files[0]); setResubmitPreview(URL.createObjectURL(e.target.files[0])) }
                                }} />
                            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
                ...
                {/* ===== TAB 1: Info (exact copy of create page) ===== */}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                        ...
                                        <div className="flex-1 space-y-2">
                                            <p className="text-sm text-gray-500">اختر صورة معبرة عن الدورة التدريبية. يفضل أن تكون بصيغة JPG أو PNG وبجودة عالية.</p>
                ...
                <TabsContent value="pricing" className="space-y-6">
                    ...
                    {/* ─── حالة 1: دورة حضورية (مفعّلة أو غير مسودة) → قفل كامل ─── */}
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    {onlineSessions.map((session, idx) => (
                                        <div key={idx} className="p-4 border rounded-lg space-y-3 bg-gray-50/60 relative">
                    ...
                    {/* ─── حالة 3: مسودة أو اكتمل الحد الأدنى → كل شيء قابل للتعديل ─── */}
                    {canSetupSchedule && (
                        <>
                                        ...
                                        <Card>
                                            <CardHeader>
                                                <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
                                                    <CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-blue-600" />اختيار المواعيد</CardTitle>
                                                    {selectedSessions.length > 0 && (
                                                        <Badge className="bg-blue-100 text-blue-800 self-start md:self-auto">
                                                            {selectedSessions.length} جلسة مختارة
                                                        </Badge>
                                                    )}
                                                </div>
                                            </CardHeader>
                                             <CardContent className="space-y-6">
                                                ...
                                                <div>
                                                    <div className="flex justify-between items-center mb-4">
                                                        <h4 className="font-semibold text-gray-700">{monthLabel}</h4>
                                                        <div className="flex gap-2">
                                                        ...
                                                        {calendarDays.map((d, i) => {
                                                            ...
                                                            return (
                                                                <button key={i} type="button"
                                                                    ...
                                                                    disabled={d.isPast}
                                                                    className={`h-10 rounded-lg text-sm font-medium transition-all ${isSelected ? 'bg-blue-600 text-white shadow' : d.isPast ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : hasSessions ? 'bg-blue-100 text-blue-800 border border-blue-300' : 'bg-white border hover:bg-gray-50'}`}
                                                                >
                                                                    {d.day}
                                                                </button>
                                                            )
                                                ...
                                                {selectedDate && (
                                                    <div className="border-t pt-4">
                                                        <h4 className="font-semibold mb-3 text-gray-700">الأوقات المتاحة ليوم {formatDateLabel(selectedDate)}</h4>
                                                        {isSlotsLoading ? (
                                                                ...
                                                                {availableSlots.map(slot => {
                                                                    ...
                                                                    return (
                                                                        <button key={slot} type="button"
                                                                            ...
                                                                            className={`p-2 text-sm rounded-lg border transition-all ${isSel ? 'bg-blue-600 text-white border-blue-600 shadow' : 'bg-white hover:bg-blue-50 hover:border-blue-300'}`}
                                                                        >
                                                                            {slot}
                                                                        </button>
                                                                    )
                                                            ...
                                                            <p className="text-sm text-gray-500 text-center py-4 bg-gray-50 rounded-lg">{unavailableMessage || "لا يوجد أوقات متاحة في هذا اليوم"}</p>
                                                        )}
                                                    </div>
                                                )}
                                                ...
                                                {selectedSessions.length > 0 && (
                                                    <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl">
                                                        <div className="flex justify-between items-center mb-3">
                                                            <h4 className="font-bold text-blue-800">تم اختيار {selectedSessions.length} جلسة</h4>
                                                            <span className="text-blue-900 font-bold">{totalPrice.toLocaleString()} ر.ي</span>
                                                        </div>
                                                        <div className="flex flex-wrap gap-2">
                                                            {selectedSessions.map((s, i) => (
                                                                <Badge key={i} variant="secondary" className="bg-white text-blue-800 gap-1">
                                                                    {formatDateLabel(s.date)} ({s.slot})
                                                                    <X className="h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => toggleSessionSlot(s.date, s.slot)} />
                                                                </Badge>
                                                            ))}
                                                        </div>
                                                    </div>
                            ...
                            {/* Online: Platform + Sessions — shown via Case 2 above, hidden here to avoid duplication */}
